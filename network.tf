resource "oci_core_virtual_network" "ClusterVCN2" {
  cidr_block     = "10.0.0.0/16"
  compartment_id = "${var.compartment_ocid}"
  display_name   = "ClusterVCN2"
  dns_label      = "clustervcn2"
}
...
resource "oci_core_subnet" "ClusterSubnet" {
  ...
  security_list_ids   = ["${oci_core_virtual_network.ClusterVCN2.default_security_list_id}", "${oci_core_security_list.ClusterSecurityList.id}"]
  compartment_id      = "${var.compartment_ocid}"
  vcn_id              = "${oci_core_virtual_network.ClusterVCN2.id}"
  route_table_id      = "${oci_core_route_table.ClusterRT.id}"
  dhcp_options_id     = "${oci_core_virtual_network.ClusterVCN2.default_dhcp_options_id}"
}
